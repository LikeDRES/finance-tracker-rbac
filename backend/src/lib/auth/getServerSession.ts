...
import { prisma } from "@/lib/db/prisma"; // Importamos prisma
...
export async function getServerSession(req: NextApiRequest): Promise<Session | null> {
  ...
  try {
    ...
    const sessionWithRole: Session = {
      ...
      user: {
        ...session.user,
        role: dbUser?.role || "USER", // Aseguramos que tenga role
      },
    } as Session;
    
    console.log("🔐 Session with role:", {
      userId: sessionWithRole.user.id,
